...
def test_run_gh_timeout_is_passed_and_raises_when_it_elapses():
    import subprocess

    with (
        patch("subprocess.run", side_effect=subprocess.TimeoutExpired(["gh"], 5)) as mock,
        pytest.raises(RuntimeError, match="timed out after 5s"),
    ):
        run_gh("pr", "list", check=False, timeout=5)
    assert mock.call_args.kwargs["timeout"] == 5


def test_run_gh_without_timeout_waits():
    with patch("subprocess.run") as mock:
        mock.return_value.returncode = 0
        run_gh("auth", "status")
...
def test_gh_pr_heads_forwards_timeout_and_is_none_when_it_elapses():
    from reinicorn.github import PR_LIST_STATE_ALL, gh_pr_heads

    with patch("reinicorn.github.run_gh", side_effect=RuntimeError("timed out")) as mock:
        assert gh_pr_heads("o/r", state=PR_LIST_STATE_ALL, timeout=7) is None
